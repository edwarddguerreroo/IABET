"""
Trainer Completo para Modelo NBA HalfTime Points
=============================================

Trainer que integra carga de datos, entrenamiento del modelo de puntos por equipo en la mitad del partido
...
- Métricas detalladas específicas para predicción de puntos por equipo en la mitad del partido
...
class HalfTimePointsTrainer:
    """
    Trainer completo para modelo de predicción de puntos por equipo NBA en la mitad del partido.
    ...
    def __init__(self,
                ...
                output_dir: str = "app/architectures/basketball/results/halftime_points_model",
                n_trials: int = 25,  # Trials para optimización bayesiana
                cv_folds: int = 5,
                random_state: int = 42):
        """
        Inicializa el trainer completo para predicción de puntos por equipo en la mitad del partido.
        
        Args:
            players_total_path: Ruta a datos de partidos
            players_quarters_path: Ruta a datos de partidos en la mitad del partido
        ...
        try:
            ...
            self.output_dir = os.path.normpath("results_halftime_points_model")
            os.makedirs(self.output_dir, exist_ok=True)
        ...
        logger.info(f"Trainer HalfTime Points inicializado")
    ...
    def load_and_prepare_data(self) -> pd.DataFrame:
        ...
        if 'HT' not in teams_data.columns:
            raise ValueError("Columna 'HT' no encontrada en datos de equipos")
        ...
        ht_stats = teams_data['HT'].describe()
        logger.info(f"Estadísticas HT por equipo:")
        logger.info(f"  | Media: {ht_stats['mean']:.1f}")
        logger.info(f"  | Mediana: {ht_stats['50%']:.1f}")
        logger.info(f"  | Min/Max: {ht_stats['min']:.0f}/{ht_stats['max']:.0f}")
        logger.info(f"  | Desv. Estándar: {ht_stats['std']:.1f}")
    ...
    def train_model(self) -> Dict:
        ...
        """
        logger.info("Iniciando entrenamiento del modelo HalfTime Points")
        ...
        logger.info(f"Modelo HalfTime Points completado")
        
        # Mostrar resultados del entrenamiento
        logger.info("=" * 50)
        logger.info("RESULTADOS DEL ENTRENAMIENTO HALFTIME POINTS")
        logger.info("=" * 50)
    ...
    def generate_all_visualizations(self):
        ...
        fig.suptitle('Dashboard Completo - Modelo NBA HalfTime Points Prediction', fontsize=20, fontweight='bold', y=0.98)
    ...
    def _plot_model_metrics_summary(self, ax):
        ...
        metrics_text = f"""
MÉTRICAS DEL MODELO HALFTIME POINTS
...
"""
    ...
    def _plot_target_distribution_compact(self, ax):
        """Distribución compacta del target HT."""
        ht_values = self.df['HT']
        
        # Histograma
        ax.hist(ht_values, bins=25, alpha=0.7, color='lightcoral', edgecolor='black')
        
        # Estadísticas
        mean_ht = ht_values.mean()
        median_ht = ht_values.median()
        
        ax.axvline(mean_ht, color='red', linestyle='--', label=f'Media: {mean_ht:.1f}')
        ax.axvline(median_ht, color='blue', linestyle='--', label=f'Mediana: {median_ht:.1f}')
        
        ax.set_xlabel('Puntos de Halftime (HT)')
        ax.set_ylabel('Frecuencia')
        ax.set_title('Distribución de HT por Equipo', fontweight='bold')
    ...
    def _plot_predictions_vs_actual_compact(self, ax):
        ...
        ax.set_xlabel('HT Real')
        ax.set_ylabel('HT Predicho')
    ...
    def _plot_top_offensive_teams_compact(self, ax):
        ...
        if len(top_teams) > 0:
            ...
            ax.set_xlabel('Promedio HT')
            ax.set_title('Top 10 Equipos Ofensivos (Halftime)', fontweight='bold')
    ...
    def _plot_temporal_analysis_compact(self, ax):
        ...
        if len(monthly_stats) > 0:
            ...
            avg_ht = monthly_stats['HT']
            
            ax.plot(months, avg_ht, marker='o', linewidth=2, markersize=4)
            
            ax.set_ylabel('Promedio HT')
            ax.set_title('Promedio de Halftime por Mes', fontweight='bold')
    ...
    def _generate_comprehensive_report(self) -> Dict:
        ...
        report = {
            ...
            'data_summary': {
                ...
                'target_variable': 'HT (Halftime Points)',
    ...
    def save_results(self):
        ...
        model_path = os.path.join(joblib_dir, 'halftime_points_model.joblib')
        ...
        if self.predictions is not None:
            if hasattr(self, 'df_filtered'):
                predictions_df = self.df_filtered[['Team', 'Date', 'Opp', 'HT']].copy()
            else:
                predictions_df = self.df[['Team', 'Date', 'Opp', 'HT']].copy()
                # Ajustar dimensiones si es necesario
            ...
            predictions_df['HT_predicted'] = self.predictions
            predictions_df['error'] = self.predictions - predictions_df['HT']
            predictions_df['abs_error'] = np.abs(predictions_df['error'])
            
            predictions_path = os.path.normpath(os.path.join(self.output_dir, 'predictions.csv'))
            predictions_df.to_csv(predictions_path, index=False)
        
        # Guardar feature importance COMPLETA (todas las features como en HT)
        try:
            if hasattr(self.model, 'get_feature_importance'):
        ...
        files_summary = {
            'model_file': os.path.abspath('app/architectures/basketball/.joblib/halftime_points_model.joblib'),
...
def main():
    ...
    trainer = HalfTimePointsTrainer(
        players_total_path=players_total_path,
        players_quarters_path=players_quarters_path,
        teams_total_path=teams_total_path,
        teams_quarters_path=teams_quarters_path,
        biometrics_path=biometrics_path,
        output_dir="app/architectures/basketball/results/halftime_points_model",
        n_trials=25,
        cv_folds=5,
        random_state=42
    )
