...
- Feature engineering especializado para puntos de equipo
"""
...
class TeamPointsNeuralNet(nn.Module):
    ...
    def __init__(self, input_size: int, hidden_sizes: List[int] = None):
        super(TeamPointsNeuralNet, self).__init__()
...
class PyTorchTeamPointsRegressor(BaseEstimator, RegressorMixin):
    ...
    def fit(self, X, y):
        ...
        self.model = TeamPointsNeuralNet(input_size, self.hidden_sizes).to(self.device)
    ...
    def predict(self, X):
        ...
        # Análisis real: Min=67, Max=176, P5=92, P95=134
        # Usando rango MODERADO (98% de casos históricos): 84-143
        predictions = np.clip(predictions, 85, 135)
        
        # MEJORA: Manejo robusto de outliers y predicciones enteras
        predictions = self._apply_robust_prediction_processing(predictions, target_type='points')
...
class TeamPointsModel:
    """
    Modelo avanzado para predicción de puntos de equipo con stacking y optimización bayesiana
    """
    ...
    def _apply_data_quality_filters(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        Q1 = df_filtered['points'].quantile(0.25)
        Q3 = df_filtered['points'].quantile(0.75)
        IQR = Q3 - Q1
        ...
        outliers_mask = (df_filtered['points'] < lower_bound) | (df_filtered['points'] > upper_bound)
        outliers_count = outliers_mask.sum()
        
        if outliers_count > 0:
            logger.warning(f"Eliminando {outliers_count} outliers extremos en puntos")
        ...
        critical_columns = ['points', 'Team', 'Date']
        ...
        impossible_points = (df_filtered['points'] < 50) | (df_filtered['points'] > 200)
        impossible_count = impossible_points.sum()
        
        if impossible_count > 0:
            logger.warning(f"Eliminando {impossible_count} registros con puntos imposibles")
        ...
        logger.info(f"  Rango de puntos final: {df_filtered['points'].min():.1f} - {df_filtered['points'].max():.1f}")
    ...
    def predict(self, df: pd.DataFrame) -> np.ndarray:
        ...
        # MEJORA: Manejo robusto de outliers y predicciones enteras
        final_predictions = self._apply_robust_prediction_processing(raw_predictions, target_type='points')
    ...
    def _apply_robust_prediction_processing(self, predictions: np.ndarray, target_type: str = 'points') -> np.ndarray:
        ...
        Adaptado específicamente para puntos de equipos NBA con mejoras para extremos.
        
        Args:
            predictions: Array de predicciones continuas
            target_type: Tipo de target ('points' para puntos de equipos)
        ...
        nba_lower = 50   # Mínimo absoluto NBA
        nba_upper = 180  # Máximo absoluto NBA
        ...
        # Para puntos de equipos, diferencias mayores a 30 puntos son raras entre juegos consecutivos
        if len(integer_predictions) > 1:
            for i in range(1, len(integer_predictions)):
                ...
                if diff > 35:  # Diferencia muy grande entre partidos consecutivos
    ...
    def save_model(self, filepath: str):
        ...
        logger.info(f"Modelo TEAMS_POINTS completo guardado: {filepath}")
    ...
    def load_model(self, filepath: str):
        ...
        try:
            ...
            if isinstance(loaded_model, TeamPointsModel):
                # Copiar todos los atributos del modelo cargado
                self.__dict__.update(loaded_model.__dict__)
                logger.info(f"Modelo TEAMS_POINTS completo cargado: {filepath}")
            else:
                ...
                if isinstance(loaded_model, dict) and 'best_model' in loaded_model:
                    ...
                    self.feature_engineer = loaded_model.get('feature_engineer', TeamPointsFeatureEngineer())
                    ...
                    logger.info(f"Modelo TEAMS_POINTS legacy cargado: {filepath}")
                else:
                    raise ValueError("Formato de modelo no reconocido")
        except Exception as e:
            logger.error(f"Error cargando modelo TEAMS_POINTS: {e}")
            raise